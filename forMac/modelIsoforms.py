...
def init(configFile):
    ...
    print "upload paths for librairies"
    ...
    return HHBLITS, ADDSS, HHMAKE, HHSEARCH, HHMODEL, PROCHECK, NACCESS, HHDB, STRUCTDB, ALLPDB, NCPU, CONTEXTLIB
...
#a partir d'une banque de transcrit, ecrit chaque transcrit dans un fichier en ne retenant que son nom et la sequence 
def parse (fic) :
...
def writePirMul(fic,seqs,borders=[]):
	if borders == []:
		...
		endstr = "% 4d" % end
		newNam = seqs[0][0].split('\n')[0] + '\n' + secondLine[0] + ':' + secondLine[1] + ':' + startstr + ':' + secondLine[3] + ':' + endstr 
...
def splitChainsPDB(fic,code,ext):
	...
	while count<nbLines:
	    ...
	    while (ligne.startswith("ATOM")==False) and (count<nbLines) :
	        try: ligne = lines[count]
	        except: print "can't read line"
	        count+=1    
		...
		wfile2=open(ficOUT, "w")
	        ...
	        count+=1    
	        while (ligne[21]==chain) and ( (ligne.startswith("ATOM")) or (ligne.startswith("HETATM")) or (ligne.startswith("ANISOU")) ) and 	(count<nbLines) :
				wfile2.writelines(ligne)
				try: ligne = lines[count] 
				except: print "can't read line"
				count+=1   
...
def assessQuality(PROCHECK,prot,pref):
	...
	with open(pref+'.B99990001.sum','r') as f:
		...
		for line in lines:
			l = line.split()
			if 'G-factors' in l:
				dihedrals, covalent, overall = l[3],l[5],l[7]
...
def model3D(fic,ALLPDB):
	...
	for i in range(1,len(seqs)):
		...
		if not os.path.isfile('pdb'+kn+'.ent'):
			copyfile(ALLPDB + nam, "./" + nam)
			os.system("gunzip ./" + nam )
			splitChainsPDB('pdb'+kn+'.ent',kn,'pdb') 
	knowns = tuple(knowns)

	a = automodel(env, alnfile  = fic, knowns= knowns, sequence = seq)
...
def runModelProcess(HHBLITS,ADDSS,HHMAKE,HHSEARCH,HHMODEL,HHDB,STRUCTDB,ALLPDB,NCPU,trans,selTemp,only3D):
	try:
		tmp = trans[2:].split('.')[0]
		if not only3D:
			# look for homologs and create the MSA alignment
			os.system(HHBLITS + " -cpu " + NCPU + " -i " + trans + " -d "+ HHDB + " -oa3m " + tmp + ".a3m -n 1")
			...
			# search for homologs 
			os.system(HHSEARCH + " -cpu " + NCPU + " -v 1 -i " + tmp + ".hhm -d " + STRUCTDB + " -o " + tmp + ".hhr -p 20 -P 20 -Z 100 -B 100 -seq 1 -aliw 80 -local -ssm 2 -norealign -sc 1 -dbstrlen 10000 -cs " + CONTEXTLIB)
