...
def runModelProcess(HHBLITS, ADDSS, HHMAKE, HHSEARCH, HHMODEL, HHDB,
    ...
    try:
        ...
        if not only3D:
            ...
            subprocess.call([HHBLITS, "-cpu", NCPU, "-i", trans,
            ...
            # search for homologs
            subprocess.call([HHSEARCH, "-cpu", NCPU, "-v", "1", "-i ",
                             tmp + ".hhm", "-d", STRUCTDB, "-o",
                             tmp + ".hhr", "-p", "20", "-P", "20",
                             "-Z", "100", "-B", "100", "-seq", "1",
                             "-aliw", "80", "-local", "-ssm", "2",
                             "-norealign", "-sc", "1", "-dbstrlen",
                             "10000", "-cs",
                             CONTEXTLIB])  # TODO : Is CONTEXTLIB defined?
        # create the alignment for modeller
        subprocess.call([HHMODEL, "-i", tmp + ".hhr",
                         "-pir", tmp + ".pir", "-m", selTemp])
