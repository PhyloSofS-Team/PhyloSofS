...
function get_exons(query_name)
    ...
    lines = readlines("$(query_name)_$(iteration).exons_lengths.txt")
    exons = [parse(Int, ss) for ss in split(lines[2])]
    # MYO1B example:
    # lines[1] e.g. ">P1;ENSG00000128641 ENST00000304164 %()+0123456789:<=?" *
    # "ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^`abcdefghijklmnopqrstuvwxyz{|}~¼½¾¿ÀÁÂÃ" *
    # "ÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞ"
    exons_symbol = split(lines[1])[3]
    # exons_symbol e.g. "%()+0123456789:<=?ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^`ab" *
    # "cdefghijklmnopqrstuvwxyz{|}~¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞ"
    for (i, exon) in enumerate(exons_symbol)
        if i == length(exons)
            break
        end
        push!(df, (exon, (exons[i+1] - exons[i])))
    end
    return df
end
...
last_pos_pir = Int64[]
fragments_templates,
fragments_identity_mean,
fragments_identity_max,
fragments_templates_names,
alltemplates,
loop_end,
loop_start,
first_pos_pir,
last_pos_pir = get_infos(exons, msa_total, f2p)
n = exons_filtered.symbol
global it = 0
for i = 1:loop_end-loop_start+1
#     println(i)
#      if exons.length[i] > 7
    if (fragments_identity_max[i] < 25)
        push!(
            ...
            (
             loop_start + it,
             Char(exons.symbol[loop_start+it]),
             fragments_templates[i],
             fragments_identity_mean[i],
             fragments_identity_max[i],
             "red",
             "0-24.99%",
             first_pos_pir[i],
             last_pos_pir[i],
             fragments_templates_names[i],
            ),
        )#, mean_rmsd[i]))
    elseif ((fragments_identity_max[i] >= 25) &&
            (fragments_identity_max[i] < 50))
        push!(
            df,
            (
             loop_start + it,
    ...
    elseif ((fragments_identity_max[i] >= 50) &&
            (fragments_identity_max[i] < 75))
        push!(
            df,
            (
             loop_start + it,
...
lines = readlines("$(query_name)_annotated.pir")


collected_exons_symbol = collect(lines[2])
first = findfirst(isequal(iterative_number.symbol[1]), collected_exons_symbol)
last = findlast(
    isequal(iterative_number.symbol[end]),
    collected_exons_symbol,
)

global exons_numbers = ""
for i = 1:length(iterative_number.symbol)
    global exons_numbers = exons_numbers *
                           (iterative_number.number[i]^((findlast(
        isequal(iterative_number.symbol[i]),
        lines[2],
    ) + 1) - findfirst(
        isequal(iterative_number.symbol[i]),
        lines[2],
